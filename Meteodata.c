...
int GetMeteoData(Weather* meteo)
{
    ...
    for (i = 0; i < WEATHER_NTYPES; i++) {
        ...
        if ((retval = nc_inq_dimid(ncid, "lat", &lat_dimid))) ERR(retval);
        if ((retval = nc_inq_dimid(ncid, "lon", &lon_dimid))) ERR(retval);
        if ((retval = nc_inq_dimid(ncid, "time", &time_dimid))) ERR(retval);
        if ((retval = nc_inq_dimlen(ncid, lat_dimid, &lat_length))) ERR(retval);
        if ((retval = nc_inq_dimlen(ncid, lon_dimid, &lon_length))) ERR(retval);
        if ((retval = nc_inq_dimlen(ncid, time_dimid, &time_length))) ERR(retval);
    
        /* ADD THIS LINE to print the discovered dimensions */
        printf("DEBUG: Found dimensions for %s: lon=%zu, lat=%zu, time=%zu\n", meteo->file[i], lon_length, lat_length, time_length);
    ...
    for (j = 0; j < lon_length; j++) {
        for (k = 0; k < lat_length; k++) {
            ...
            for (l = 0; l < time_length; l++) {
                ...
                Rain[j][k][l] = roundz(10 * Rain[j][k][l], 2); // [cm day-1]
