...
void CalcPenmanMonteith()
{
    ...
    Svap_Tmin = 0.6108 * exp((17.27 * Tmin[Day]) / (237.3 + Tmin[Day]));
    ...
    Stb_Tmax = Stbc * pow((273.16 + Tmax[Day]), 4);
    Stb_Tmin = Stbc * pow((273.16 + Tmin[Day]), 4);
    ...
    if (CskyRad > 0)
    {
        ...
        Rnl = Rnl_Tmp * (1.35 * (Radiation[Day]/CskyRad) - 0.35);
